...
import {
  Plus, ArrowLeft, Clock, CheckCircle2, BookOpen,
  Trash2, Pencil, Check, X, ChevronRight, Settings,
} from "lucide-react";
...
function SessionCard({
  ...
  return (
    <div
      className="group relative rounded-xl border bg-card hover:border-primary/60 hover:shadow-md transition-all cursor-pointer flex flex-col overflow-hidden"
      onClick={() => navigate(`/ccl/${session.sessionId}`)}
    >
      {/* coral top accent rule on hover */}
      <span
        aria-hidden="true"
        className="absolute top-0 left-0 right-0 h-[3px] bg-primary origin-left scale-x-0 group-hover:scale-x-100 transition-transform"
      />
      ...
      <div className="flex-1 p-5">
        ...
        <div className="flex items-start gap-2 mb-3 pr-16">
          {editing ? (
            ...
            <h3 className="font-display font-bold text-base text-foreground leading-snug tracking-tight">
              {displayName(session)}
            </h3>
          )}
        </div>
        ...
        <div className="flex flex-wrap gap-1.5 mb-4">
          {session.gradeBand && (
            <span className="text-[10px] font-display font-bold uppercase tracking-[0.12em] bg-primary/10 text-primary px-2 py-0.5 rounded-full">
              {session.gradeBand}
            </span>
          )}
          {complete ? (
            <span className="text-[10px] font-display font-bold uppercase tracking-[0.12em] bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded-full flex items-center gap-1">
              <CheckCircle2 className="w-3 h-3" />
              Complete
            </span>
          ) : (
            <span className="text-[10px] font-display font-bold uppercase tracking-[0.12em] bg-muted text-muted-foreground px-2 py-0.5 rounded-full flex items-center gap-1">
      ...
      <div className="flex items-center justify-between px-5 py-3 border-t bg-muted/20">
        ...
        <div className="flex items-center gap-1 text-[10px] font-display font-bold uppercase tracking-[0.14em] text-primary opacity-0 group-hover:opacity-100 transition-opacity">
...
function NewSessionDialog({
  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center px-4">
      <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6 border-t-[3px] border-primary">
        <p className="t-eyebrow mb-2">New questionnaire</p>
        <h2 className="font-display font-bold uppercase tracking-tight text-xl mb-1">Start a new questionnaire</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="text-[11px] font-display font-bold uppercase tracking-[0.14em] text-foreground block mb-1.5">
              District <span className="text-muted-foreground font-normal normal-case tracking-normal">(optional)</span>
            </label>
            <input
              value={district}
...
function DeleteDialog({
  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center px-4">
      <div className="bg-white rounded-xl shadow-xl max-w-sm w-full p-6 border-t-[3px] border-destructive">
        <h2 className="font-display font-bold uppercase tracking-tight text-lg mb-2">Delete questionnaire?</h2>
        <p className="text-sm text-muted-foreground mb-5">
      ...
      <main className="flex-1 max-w-5xl w-full mx-auto px-6 py-10">
        ...
        <div className="flex items-start justify-between gap-4 mb-10">
          <div>
            <button
              onClick={() => navigate("/")}
              className="flex items-center gap-1.5 text-[11px] font-display font-bold uppercase tracking-[0.14em] text-muted-foreground hover:text-foreground mb-4 transition-colors"
            >
              <ArrowLeft className="w-3.5 h-3.5" />
              All focus areas
            </button>
            <p className="t-eyebrow mb-2">Focus area</p>
            <h1 className="t-display text-3xl md:text-4xl">Career Connected Learning</h1>
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <h2 className="font-display font-bold uppercase tracking-tight text-xl mb-2">No questionnaires yet</h2>
